...
TEST_CASE("LV Image functionality RGB888 blend to RGB888", "[image][functionality][RGB888]")
{
    ...
    func_test_case_lv_image_params_t test_case = {
        .blend_api_func_px_size =
            &lv_draw_sw_blend_image_to_rgb888, // The blending API function takes additional parameter, pixel size
        .color_format = LV_COLOR_FORMAT_RGB888,
        .canary_pixels = CANARY_PIXELS_RGB888,
        .memory_alignment_offset =
            32 - (CANARY_PIXELS_RGB888 * 3), // Closes 16-byte boundary (32) - RGB888 canary pixels
    ...
    for (int dest_w = test_matrix->min_w; dest_w <= test_matrix->max_w; dest_w++) {
        ...
        for (int dest_h = test_matrix->min_h; dest_h <= test_matrix->max_h; dest_h++) {
            ...
            for (int src_stride = dest_w; src_stride <= dest_w * 2; src_stride += test_matrix->src_stride_step) {
                ...
                for (int dest_stride = dest_w; dest_stride <= dest_w * 2;
                     dest_stride += test_matrix->dest_stride_step) {

                    // Step source array unalignment
                    for (int src_unalign_byte = test_matrix->src_min_unalign_byte;
                         src_unalign_byte <= test_matrix->src_max_unalign_byte;
                        ...
                        for (int dest_unalign_byte = test_matrix->dest_min_unalign_byte;
                             dest_unalign_byte <= test_matrix->dest_max_unalign_byte;
                             dest_unalign_byte += test_matrix->dest_unalign_step) {

                            // Call functionality test
                            UPDATE_TEST_CASE(test_case, dest_w, dest_h, src_stride, dest_stride, src_unalign_byte,
                                             dest_unalign_byte);
...
static void lv_image_functionality(func_test_case_lv_image_params_t *test_case)
{
    ...
    sprintf(test_msg_buf,
            "Test case: dest_w = %d, dest_h = %d, dest_stride = %d, src_stride = %d, dest_unalign_byte = %d, "
            "src_unalign_byte = %d\n",
            test_case->dest_w, test_case->dest_h, test_case->dest_stride, test_case->src_stride,
            test_case->dest_unalign_byte, test_case->src_unalign_byte);
#if DBG_PRINT_OUTPUT
    printf("%s\n", test_msg_buf);
#endif
...
static void fill_test_bufs(func_test_case_lv_image_params_t *test_case)
{
    ...
    const size_t src_buf_len =
        test_case->src_buf_len; // Total source buffer length, data part of the source buffer including matrix padding
                                // (no Canary pixels are used for source buffer)
    const size_t total_dest_buf_len =
        test_case->total_dest_buf_len; // Total destination buffer length, data part of the destination buffer including
                                       // the Canary pixels and matrix padding
    ...
    const unsigned int memory_offset =
        test_case->memory_alignment_offset; // Memory alignment offset for 16-byte boundary
    ...
    for (uint32_t i = 0; i < test_case->src_buf_len; i++) {
        printf("src_buf[%" PRIi32 "] %s = %8" PRIx16 " \n", i, ((i < 10) ? (" ") : ("")),
               ((uint16_t *)test_case->buf.p_src)[i]);
    }
    printf("\n");
#endif
}
...
static void test_eval_image_16bit_data(func_test_case_lv_image_params_t *test_case)
{
    ...
    for (uint32_t i = 0; i < test_case->total_dest_buf_len; i++) {
        printf("dest_buf[%" PRIi32 "] %s ansi = %8" PRIx16 " \t asm = %8" PRIx16 "   %s \n", i,
               ((i < 10) ? (" ") : ("")), ((uint16_t *)test_case->buf.p_dest_ansi)[i],
               ((uint16_t *)test_case->buf.p_dest_asm)[i],
               (((uint16_t *)test_case->buf.p_dest_ansi)[i] == ((uint16_t *)test_case->buf.p_dest_asm)[i]) ? ("OK")
                                                                                                           : ("FAIL"));
    }
    printf("\n");
    ...
    TEST_ASSERT_EQUAL_UINT16_ARRAY_MESSAGE((uint16_t *)test_case->buf.p_dest_ansi + canary_pixels,
                                           (uint16_t *)test_case->buf.p_dest_asm + canary_pixels,
                                           test_case->active_dest_buf_len, test_msg_buf);
    ...
    TEST_ASSERT_EACH_EQUAL_UINT16_MESSAGE(
        0, (uint16_t *)test_case->buf.p_dest_ansi + (test_case->total_dest_buf_len - canary_pixels), canary_pixels,
        test_msg_buf);
    TEST_ASSERT_EACH_EQUAL_UINT16_MESSAGE(
        0, (uint16_t *)test_case->buf.p_dest_asm + (test_case->total_dest_buf_len - canary_pixels), canary_pixels,
        test_msg_buf);
}
...
// Print results, 24bit data
    ...
    TEST_ASSERT_EQUAL_UINT8_ARRAY_MESSAGE((uint8_t *)test_case->buf.p_dest_ansi + (canary_pixels * 3),
                                          (uint8_t *)test_case->buf.p_dest_asm + (canary_pixels * 3),
                                          test_case->active_dest_buf_len, test_msg_buf);
    ...
    TEST_ASSERT_EACH_EQUAL_UINT8_MESSAGE(
        0, (uint8_t *)test_case->buf.p_dest_ansi + ((test_case->total_dest_buf_len * 3) - (canary_pixels * 3)),
        canary_pixels * 3, test_msg_buf);
    TEST_ASSERT_EACH_EQUAL_UINT8_MESSAGE(
        0, (uint8_t *)test_case->buf.p_dest_asm + ((test_case->total_dest_buf_len * 3) - (canary_pixels * 3)),
        canary_pixels * 3, test_msg_buf);
}
