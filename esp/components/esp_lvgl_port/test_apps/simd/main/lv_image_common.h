 ...
 *    - The memory occupied by canary pixels should be in 16-byte multiples, to achieve 16-byte memory alignment in
 * functionality test
...
typedef struct {
    struct {
        ...
        void *p_src_alloc; /*!< pointer to the beginning of the memory allocated for the source ASM test buf, used in
                              free() */
        void *p_dest_asm;  /*!< pointer to the destination ASM test buf */
        void *p_dest_ansi; /*!< pointer to the destination ANSI test buf */
        void *p_dest_asm_alloc;  /*!< pointer to the beginning of the memory allocated for the destination ASM test buf,
                                    used in free() */
        void *p_dest_ansi_alloc; /*!< pointer to the beginning of the memory allocated for the destination ANSI test
                                    buf, used in free() */
    } buf;
    void (*blend_api_func)(_lv_draw_sw_blend_image_dsc_t *); /*!< pointer to LVGL API function */
    void (*blend_api_func_px_size)(
    ...
    size_t src_buf_len; /*!< Length of the source buffer, including matrix padding (no Canary pixels are used for source
                           buffer) */
    size_t active_dest_buf_len; /*!< Length of the destination buffer, where the actual data are stored, including
                                   matrix padding, not including Canary pixels */
    size_t
        total_dest_buf_len; /*!< Total length of the destination buffer (including Canary pixels and matrix padding) */
    size_t canary_pixels;   /*!< Canary pixels must be adjusted according to the used color type, to achieve aligned
                               memory effect */
