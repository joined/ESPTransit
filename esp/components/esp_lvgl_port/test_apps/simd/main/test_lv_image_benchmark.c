...
Procedure:
    ...
    - Firstly use an input test parameters for the most ideal case (16-byte aligned arrays, arrays widths divisible by 2
for RGB565 color format)
    - Then use worst-case input test parameters (1-byte aligned arrays, arrays width NOT divisible by 2 for RGB565 color
format)
    - Count how many CPU cycles does it take to run a function from the LVGL blending API for each case (ideal and worst
case)
    - Run ansi version of LVGL blending API multiple times (1000-times or so) and repeat the 2 above steps for the ansi
version
    - Compare the results
...
Inducing Most ideal and worst case scenarios:
    - Most ideal:
        - Both, the source and the destination buffers should be aligned by 16-byte (Xtensa PIE), or 4-byte (Xtensa
base) boundaries
        ...
        - Matrix height does not have any effect on benchmark unit tests, unit the matrix is too large that cache
limitations start to affect the performance
        - Matrix strides, should be equal to the matrix widths (0 matrix padding), or their multiples (matrix width =
matrix padding)
    - Worst case:
        - Both, hte source and the destination buffers should NOT be aligned by 16-byte (Xtensa PIE), or 4-byte (Xtensa
base) boundaries, Source buffer unalignment should be different from the destination unalignment, with one unalignment
being even, the other being odd The unalignments shall be small numbers (preferably 1 or 2 bytes)
...
static void lv_image_benchmark_init(bench_test_case_lv_image_params_t *test_params)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        ESP_LOGI(TAG_LV_IMAGE_BENCH,
                 " %s ideal case: %.3f cycles for %" PRIi32 "x%" PRIi32 " matrix, %.3f cycles per sample",
        ...
        ESP_LOGI(TAG_LV_IMAGE_BENCH,
                 " %s corner case: %.3f cycles for %" PRIi32 "x%" PRIi32 " matrix, %.3f cycles per sample\n",
