...
lv_indev_t *lvgl_port_add_navigation_buttons(const lvgl_port_nav_btns_cfg_t *buttons_cfg)
{
...
#else
    ESP_GOTO_ON_FALSE(buttons_cfg->button_prev && buttons_cfg->button_next && buttons_cfg->button_enter,
                      ESP_ERR_INVALID_ARG, err, TAG, "Invalid some button handler!");
    ...
    for (int i = 0; i < LVGL_PORT_NAV_BTN_CNT; i++) {
        ...
        ESP_ERROR_CHECK(iot_button_register_cb(buttons_ctx->btn[i], BUTTON_PRESS_DOWN, NULL, lvgl_port_btn_down_handler,
                                               buttons_ctx));
        ESP_ERROR_CHECK(
            iot_button_register_cb(buttons_ctx->btn[i], BUTTON_PRESS_UP, NULL, lvgl_port_btn_up_handler, buttons_ctx));
#endif
    }
