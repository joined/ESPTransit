...
static void lvgl_port_flush_callback(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    if (disp_ctx->trans_size == 0) {
        ...
        if (disp_ctx->disp_type == LVGL_PORT_DISP_TYPE_RGB ||
            (disp_ctx->disp_type == LVGL_PORT_DISP_TYPE_DSI && (drv->direct_mode || drv->full_refresh))) {
        ...
        for (int i = 0; i < trans_count; i++) {
            ...
            esp_lcd_panel_draw_bitmap(disp_ctx->panel_handle, x_draw_start, y_draw_start, x_draw_end + 1,
                                      y_draw_end + 1, to);
